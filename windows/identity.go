...
//
// Stored as plain files under %LOCALAPPDATA%\Remotype Host, matching the Mac
// host's decision to use a mode-0600 file rather than the platform keystore.
// The reasoning is the same on both: a credential store keyed to the code
// signature loses every pairing on an update, and the threat model here is "a
// peer on your Wi-Fi", not "an attacker who can already read your files as
// you". DPAPI would add real protection against the second, and nothing against
// the first — so it is a future refinement, not a prerequisite.
//
...
// readable by everyone; on Windows the mode is advisory, but the ACL inherited
// from %LOCALAPPDATA% is already per-user.
