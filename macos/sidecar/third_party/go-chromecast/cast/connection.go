...
func (c *Connection) handleMessage(requestID int, message *pb.CastMessage, headers *PayloadHeader) {
	...
	if err != nil {
		// PATCH (Remotype): a CUSTOM-namespace message is application-defined and
		// need not carry Google's "type" key — our Cast receiver replies with
		// {"kind":"answer"|"ice"|"ready", ...}. Upstream dropped these on the
		// floor here, so the WebRTC answer never reached the sender and every
		// cast hung in "connecting" forever. Forward them instead of discarding.
