...
func withMods(mods int, f func()) {
	var applied []uint16
	for _, b := range []int{modCtrl, modShift, modAlt, modGUI, modAltGr} {
...
// typeChar types a single character with the given latched mods. The key and
// the level modifiers (Shift, AltGr) come from the character itself: the
// built-in US table when that is the layout, otherwise xkbcommon's answer for
// the active layout (layout.go).
func typeChar(c string, mods int) {
	...
	extra := mods
	if layoutIsUS() {
		usCode, shift, ok := asciiKey(r)
		if !ok {
			layoutNoteUnreachable(r)
			return
		}
		code = usCode
		if shift {
		...
		seq, ok := layoutSequence(r)
		if !ok {
			return
		}
		// A dead-key route is two presses — the accent, then the letter — and
		// each carries its own level modifiers.
		for _, kp := range seq {
			withMods(extra|kp.mods, func() { tap(kp.code) })
		}
		return
	}
...
// layoutNoteUnreachable logs once per character the US table cannot type —
// anything outside printable ASCII — so a silently dropped ñ is at least a
// line in the log pointing at -layout.
func layoutNoteUnreachable(r rune) {
	layoutMu.Lock()
	seen := layoutMissed[r]
	layoutMissed[r] = true
	layoutMu.Unlock()
	if !seen {
		logf("the US table cannot type %q (U+%04X) — skipped; start with -layout <xkb layout> for non-US typing", r, r)
	}
}
...
// allUsedKeys returns every EV_KEY code we register with UI_SET_KEYBIT. The
// kernel silently drops events for codes a device did not declare, and with
// layout-aware typing the code for a character is whatever xkbcommon answers —
// AltGr (KEY_RIGHTALT), the ISO key left of Z (KEY_102ND), keys the US tables
// never mention. So the whole keyboard range is declared, and the explicit
// sets below remain for the codes above it (media keys) and as documentation.
